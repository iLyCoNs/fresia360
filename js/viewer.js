...
function ensureStitchedDivisorias() {
    ...
    for (let i = 0; i < rects.length; i++) {
        for (let j = i + 1; j < rects.length; j++) {
            ...
            if (!rectsSameWidth(a, b, SNAP)) continue;
            let upper = null, lower = null;
            if (Math.abs(a.bottom - b.top) < SNAP) { upper = a; lower = b; }
            else if (Math.abs(b.bottom - a.top) < SNAP) { upper = b; lower = a; }
            else continue;
            const proj = getPanoramaScreenProjector();
            if (!proj) continue;
            const p1 = proj.toPY(upper.bl[0], upper.bottom), p2 = proj.toPY(upper.br[0], upper.bottom);
            if (!p1 || !p2) continue;
            const tol = 0.15;
            allDrawnLines = allDrawnLines.filter(l => {
            ...
            if (!exists) {
                allDrawnLines.push({
                    ...
                    franjaGrupo: upper.gid, franjaStitch: lower.gid
                });
            }
        }
    }
...
function commitFranjaFromModal() {
    ...
    franjaDraftCount = N;
    franjaDraftBaseM2 = Math.round(weights.reduce((a, b) => a + b, 0) / N);
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (urlParams.get('admin') === 'true') {
        ...
        setTimeout(() => { togglePinsMode(true); }, 3500);
...
function syncSVGElements() {
    ...
    if (franjaPreviewQuad) currentLineIds.push('franja_preview_quad'); franjaPreviewDivs.forEach(d => currentLineIds.push(d.id));
    ...
    if (franjaPreviewQuad) allLinesData.push({ id: 'franja_preview_quad', tipo: 'franja-preview', puntos: franjaPreviewQuad }); franjaPreviewDivs.forEach(d => allLinesData.push(d));
...
function updateSVGPaths() {
    ...
    Object.keys(DOMCache.paths).forEach(lineId => {
        ...
        if (!isClosed && (lineId === currentTempLineId || lineId === lineaPinesTempId) && window.lastMouseX !== undefined) { let mx = window.lastMouseX - DOMCache.viewport.left, my = window.lastMouseY - DOMCache.viewport.top; if(dBase === '' && pts.length > 0) { let c1 = getCam(pts[0][0], pts[0][1]); if(c1.z > 0) dBase += `M ${cx + (c1.x / c1.z) * f},${cy_screen - (c1.y / c1.z) * f} `; } dBase += `L ${mx},${my} `; hasVisiblePoints = true; }
